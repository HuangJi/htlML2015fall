...
for iteratorFor2000 in xrange(0, len(random2000Lists)):
	correctRecordNumPyVector = np.linspace(0, 0, 64 * len(rawInputNumPyVector))
	correctRecordNumPyVectorList = correctRecordNumPyVector.reshape(-1, 500)
	wNumPyVector = np.linspace(0, 0, 64 * len(rawInputNumPyVector))
	...
	while t < 50:
		...
		t += 1
	errorRate = getErrorRate(wNumPyVectorList[pocketTofW], xyTestNumPyVectorList, random2000Lists[iteratorFor2000]) 
